...
class tree:
    ...
    def create_splits(self, Y, X, Y_est, X_est):
        ...
        while len(node_list) > 0:
            ...
            if node.split_sample_inds.shape[0] >= self.min_leaf_size and depth < self.max_depth:
                ...
                (node_sol, nu0, lambda0, node_obj) = self.opt_solver(node_Y, verbose = self.verbose)
                node.obj = node_obj 
                ...
                (imp_parent, imp_left, imp_right, imp_dec) = self.impurity_computer(node_Y, node_sol, node_Y_left, node_Y_right, 
                         node_h_left, node_h_right, f_grad, g_grad, node_hessian, nu0, lambda0, best_split_ind)
    ...
    def find_impurity_decrease_opt(self, node):
        if (node.left is None) or (node.right is None):
            return []
        
        temp = node.obj * len(node.split_sample_inds)- (node.left.obj * len(node.left.split_sample_inds) + node.right.obj * len(node.right.split_sample_inds))
        imp_list = [temp] + self.find_impurity_decrease_opt(node.left) + self.find_impurity_decrease_opt(node.right)
...
class forest:
    ...
    def fit_forest(self, Y, X, Y_est, X_est):
        ...
        # def fit_forest_parallel(self, Y, X, Y_est, X_est, n_jobs = 1):
        # if self.bootstrap:
        #     if self.subsample_ratio > 1.0:
        #         # Safety check
        #         self.subsample_ratio = 1.0
        ...
        # trees = []
        # trees = Parallel(n_jobs=n_jobs, verbose = 3)(delayed(build_tree)(Y[s], X[s], Y_est, X_est, opt_solver = self.opt_solver, compute_update_step = self.compute_update_step,
        #                         gradient_computer = self.gradient_computer, hessian_computer = self.hessian_computer, 
        #                         crit_computer = self.crit_computer, search_active_constraint = self.search_active_constraint, 
        #                         honesty = self.honesty, mtry = self.mtry,
        #                         min_leaf_size = self.min_leaf_size, max_depth = self.max_depth, 
        #                         n_proposals = self.n_proposals, balancedness_tol = self.balancedness_tol, verbose = self.verbose) for s in subsample_ind)
    ...
    def compute_impurity_opt_fi(self, p):

        fi = np.zeros((len(self.trees), p))
        for i in range(len(self.trees)):
            tree = self.trees[i]
            feature_list = tree.find_feature(tree.tree)
            imp_list = tree.find_impurity_decrease_opt(tree.tree)
            for j in range(len(feature_list)):
                fi[i, feature_list[j]] += imp_list[j]
        return fi.mean(0) 
