...
def evaluate_one_run(models, X, Y, X_est, Y_est, X_test, Y_test, A_mat = None, b_vec = None, alpha = None):
    ...
    # for each method, as a key of dictionary, create a matrix
    for key in models.keys():
        decisions[key] = np.zeros((Nx_test, L))

    # for each key, for each test point, get the weights and save the optimal decisions of the forest
...
def evaluate_risk(decisions, Y_test, alpha = 0.9): # Default alpha is preset to 90%. This can be overrided by other alpha later...
